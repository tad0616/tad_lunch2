    ...
    <table class="table table-striped table-hover">
      ...
      <{foreach from=$all_content item=data}>
        <tr>
          ...
          <{if $isAdmin}>
          <td>
            <a href="javascript:delete_tad_lunch2_func(<{$data.lunch_sn}>);" class="btn btn-sm btn-danger"><{$smarty.const._TAD_DEL}></a>
            <a href="<{$action}>?op=tad_lunch2_form&lunch_sn=<{$data.lunch_sn}>" class="btn btn-sm btn-warning"><{$smarty.const._TAD_EDIT}></a>
          </td>
          <{/if}>
        </tr>
      <{/foreach}>
