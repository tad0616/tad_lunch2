...
function import_excel($lunch_sn="",$lunch_target="",$file=""){
  ...
  for ($row = 1; $row <= $highestRow; $row++) {
    ...
    for ($column = 0; $column <= 25; $column++) {
      ...
      if($column == 0 and $val==_MD_TADLUNCH2_LUNCH_DATE){
        $isTitle=true;
      }elseif($column == 0 and !preg_match('/^\d{4}[-\/]\d{1,2}[-\/]\d{1,2}$/u', $val)){
        $continue=true;
      }
